...
1.  **Verify First（情報源の検証）**: 検索結果やサードパーティリソースを採用する前に、必ず信頼性を評価する。公式リポジトリのREADMEを最初に読み、stars/pulls/DL数で定量評価し、「便利そう」だけで飛びつかない。詳細は `references/GUARDRAILS.md` セクション0 参照。
2.  **Lock（成功体験の保存）**: 成功した手順をナレッジベースに保存し、再現可能にする。
3.  **Search Better（継続的改善）**: 実行前に「より良い方法」をウェブ検索し、既存のベンチマークと比較。ただし検索結果の信頼性評価を必ず行う (原則1)。
4.  **Anti-Obsolescence（陳腐化防止）**: **MCPやOpenSkillsといった「現在の標準」すら陳腐化する可能性を常に考慮する。** 2026年現在の定石を使いつつも、より上位のパラダイムや新技術（Post-MCP, Specialized AI-Native APIs等）の有無を必ず確認し、優位性があれば自律的に採用を検討すること。
