...
1. **プリビルドイメージ（モデル入り）** — 最も信頼性が高い
   - 例: `valyriantech/ace-step-1.5` (ACE-Step 1.5 + REST API + 全モデル)
   - 起動時のダウンロード不要、ネットワーク障害の影響を受けない
2. **公式Dockerfile** — リポジトリにDockerfile/docker-compose.ymlがある場合
3. **nvidia/cuda:12.x-runtime-ubuntu22.04** — 軽量ランタイムイメージ
4. ~~nvidia/cuda:12.x-devel-ubuntu22.04~~ — **使用禁止**
...
|---------|-------|------|
| runtime | ~4GB | 推論、実行 |
| devel | ~10GB | CUDAコンパイル (nvcc) が必要な場合のみ |

推論ワークロードでは `devel` のコンパイラは不要。
イメージpull時間が2-3倍になり、キャッシュミス率も上がる。
